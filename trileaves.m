...
% function  [sc, tc] = trileaves(n, rold, stringa)
% inputs
%      n -  number of point from which draw the arc
%      rold -  outer radius of the circular band
%      stringa - The color of the lines and curves generated by the function.
% output
%      sc  - x-coordinates of the n-foil intersecting motif
%            based on a circular arc
%      tc  - y-coordinates of the n-foil intersecting motif
% MIT license 
...
for i = 1:n
    ...
    next = mod(i,n) + 1;
    prev=mod(i-2,n)+1;
    % t0 and t1 are the start and end angles for the circular arc.
    % t0 is the angle from vertex i to the previous vertex.
    % t1 is the angle from vertex i to the next vertex.
