...
% rc1 is the radius of the arc of circle that will
% be constructed on each point (xx(i),yy(i)) 
% starting from the previous point (xx(prev),yy(prev)) 
% and going to the next point (xx(next), yy(next))
rc1 = norm([xx(1) yy(1)] - [xx(2) yy(2)]);
hold on
for i = 1:n
    % next and prev are the next and previous indices to respect
    % to index i
    next = mod(i,n) + 1;
    prev=mod(i-2,n)+1;
    ...
    % we make this check to be sure that the rotation is
    % counterclockwise
    if t1< t0
        t1 = t1 + 2*pi;
    end
